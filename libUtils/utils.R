...
produceOpenOfficeOutput <- function(outPutDirName){
    print("[LOG utils.R ]:produceOpenOfficeOutput <- function(outPutDirName){");

    fullOutDirPath=paste(pathForRScriptsWorkspace,outPutDirName, sep="/");
    fullOutDirPath <- enclosePathWithSingleQuote(fullOutDirPath);

    cmd <- paste("./produce_odt_final_output.sh", fullOutDirPath);
    
    utilDir = paste(pathForRScriptsWorkspace,"util",sep="/");    
    CdUtilDir = paste(paste("cd ",enclosePathWithSingleQuote(utilDir)));
    
    runCmd = paste(CdUtilDir, cmd, sep = " && ");
    
    #cmdWithParameter <- paste(cmd,fullOutDirPath,sep=" ");
    #cmdWithParameter <- enclosePathWithSingleQuote(cmdWithParameter);
    #print(cmdWithParameter);
    system(runCmd);
    ##print(list.files(fullOutDirPath));
    ##produce_odt_final_output.sh
}
...
##Create current dir for each run.R script execution
...
cmd_pdflatext_for_tex_fragment <- function(texFragmentFilePath){
    print("[LOG utils.R]: cmd_pdflatext_for_tex_fragment <- function(texFragmentFilePath){... ");
...
##??
cmd_html_latext_for_tex_fragment <- function(texFragmentFilePath){
   ##Now we need to compose string command to pass to the system to generate pdf from latex fragmente
    FullPathToCurrentOutDir <-  enclosePathWithSingleQuote(paste(pathForRScriptsWorkspace, currentOutDir, sep="/"));
    ChangeDirTO_FullPathToCurrentOutDir = paste("cd", FullPathToCurrentOutDir);
    pdflatexShellScriptPath <- enclosePathWithSingleQuote(paste(pathForRScriptsWorkspace,"util/pdflatext_tex_fragment.sh", sep="/"));
